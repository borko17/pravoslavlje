-- Слава Богу
print("Преузимање молитвеника...")

local handle =
 io.popen("curl -s -D - https://raw.githubusercontent.com/borko17/pravoslavlje/refs/heads/main/molitvenik.json")
if not handle then
...
if not response or response == "" then
 print("Грешка: Није могуће преузети молитвеник. Провјерите интернет конекцију.")
 return
end
...
local skript_verzija = "1.4.2"
local skript_datum = "9.10.2025 9:10"
...
local function latinica_u_cirilicu(s)
 if
  not (s == "plj" or s:match("^plj%d") or s == "poslj" or s:match("^poslj%d") or s == "s,plj" or
   s:match("^s,plj%d") or
   s == "s,poslj" or
   s:match("^s,poslj%d"))
  then
...
local function safe_insert(t, val)
 local tip = type(val)
 if tip ~= "string" and tip ~= "number" and tip ~= "table" then
  return
 end
 table.insert(t, val)
end
...
local function utf8_char_at(s, i)
 ...
 if c < 0x80 then
  return s:sub(i, i), 1
 elseif c < 0xE0 then
  return s:sub(i, i + 1), 2
 elseif c < 0xF0 then
  return s:sub(i, i + 2), 3
 elseif c < 0xF8 then
  return s:sub(i, i + 3), 4
 else
  return nil
 end
end
...
print("\n=== МОЛИТВЕНИК ===")
print("Унеси скраћеницу из Молитвеника.")
...
while true do
 ...
 if novi_unos == "v" or novi_unos == "в" then
  ...
  print("\n== lua ==")
  print(
   "Тренутно користите lua верзију:\n" ..
 ...
 if novi_unos == "n" or novi_unos == "н" then
  ...
  for i = start_index, math.min(start_index + 2, #linije) do
   ...
   if global_read_mode then
    print(procisti_za_citanje(stih))
   else
  ...
  if unos == "" then
   ...
   if druga_linija then
    if global_read_mode then
     print(procisti_za_citanje(druga_linija))
    else
 ...
 if novi_unos:match("^п[,#]") then
  ...
  if ukupan_broj == 0 then
   ...
   if is_phrase_search then
    print(
     "---------------------------------\nПронађено укупно " ..
      broj_sa_imenicom(ukupan_broj, "стих", "стиха", "стихова") ..
       " где се налази фраза: '" .. search_terms[1] .. "'\n---------------------------------"
    )
   else
    print(
     "---------------------------------\nПронађено укупно " ..
      broj_sa_imenicom(ukupan_broj, "стих", "стиха", "стихова") ..
       " где се налазе ријечи: '" ..
        table.concat(search_terms, "', '") .. "'\n---------------------------------"
    )
   end
   os.execute("sleep 2")
   for _, r in ipairs(rezultati) do
    if global_read_mode then
     ...
     print(
      "[" ..
       r.odlomak ..
        "," .. r.broj_stiha .. "] " .. r.tekst:gsub("%[(.-)%]", uredi_tekst_u_zagradama)
     )
    end
   end
  end
      ...
      print(
       oznaka ..
        " " ..
         broj ..
          ". (" ..
           broj_sa_imenicom(broj_stihova, "стих", "стиха", "стихова") ..
            ")\n" .. naslov
      )
      pronadjeno = true
     end
 ...
 for _, podaci in ipairs(svi_odlomci) do
  ...
  if tekst then
   ...
   if podaci.stihovi then
    ...
    if podaci.stihovi == "н" then
     local linije = {}
     for lin in tekst:gmatch("[^\r\n]+") do
      table.insert(linije, lin)
     end
     local treca_linija = linije[4]
     local cetvrta_linija = linije[5]
     ...
     if #izabrano > 0 then
      ...
      print(
       "Одломак " ..
        podaci.odlomak ..
         " има " .. broj_sa_imenicom(broj_stihova, "стих", "стиха", "стихова") .. "."
      )
     end
    end
   else
   ...
   if knjiga then
    ...
    if najveca_glava > 0 then
     print(
      "Књига " ..
       knjiga .. " има " .. broj_sa_imenicom(najveca_glava, "главу", "главе", "глава") .. "."
     )
    end
   end
  end
